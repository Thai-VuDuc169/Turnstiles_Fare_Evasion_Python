...
from scripts.SVM.data_augmentation import augmentImages
...
BIN_IMGS_PATH = r'output/svm_dataset/bin_imgs'
AUGMENTED_RGB_IMGS_PATH = r"output/svm_dataset/augmented_rgb_imgs"
...
os.makedirs(BIN_IMGS_PATH)
os.makedirs(AUGMENTED_RGB_IMGS_PATH)
...
dataset_dict = {WALKING_FOLDER_PATH : LABELS["walking"], NOT_WALKING_FOLDER_PATH : LABELS["not_walking"]}

# // code lại chỗ này để dùng chung
try:
    print("initializing EfficientPose model!")
    efficient_pose = EfficientPose(folder_path= BIN_IMGS_PATH, model_name= "IV")
    print("===initialize EfficientPose model successfully!")
    for (folder_path, label) in dataset_dict.items():
        print("===running: folder path: '" + folder_path + "' and label: " + str(label))
        for file_name in os.listdir(folder_path):
            img = cv.imread(os.path.join(folder_path, file_name)) # rgb image
            for augm_method_name, augm_img in augmentImages(img):
                # cv.imwrite(os.path.join (AUGMENTED_RGB_IMGS_PATH, augm_method_name + r"_" + file_name), augm_img)
                temp_list = []
                coordinates = efficient_pose.genCoordinates(augm_img, augm_method_name + r"_" + file_name)
