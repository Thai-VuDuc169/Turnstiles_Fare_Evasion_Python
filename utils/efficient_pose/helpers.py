...
def preprocess(batch, resolution, lite=False):
    ...
    # Refer: https://www.tensorflow.org/api_docs/python/tf/keras/applications/imagenet_utils/preprocess_input
    if lite:
...
################### This function has been customized to fit the project!
def drawBinImage(image_draw, coordinates, image_height=1024, image_width=1024, segment_width=5 ,marker_radius=5):
    '''
    Draw markers and segments between body parts according to predicted body part locations.
    Args:
        image_draw: PIL ImageDraw module
            Module for performing drawing operations
        coordinates: List
    ...
    for (body_part_a_index, body_part_b_index) in segments:
        ...
        body_part_b_x *= image_width
        body_part_b_y *= image_height
        image_draw.line([(body_part_a_x, body_part_a_y), (body_part_b_x, body_part_b_y)], fill= 1, width=segment_width)
    for i, (_, body_part_x, body_part_y) in enumerate(coordinates):
        body_part_x *= image_width
        body_part_y *= image_height
        image_draw.ellipse([(body_part_x - marker_radius, body_part_y - marker_radius), (body_part_x + marker_radius, body_part_y + marker_radius)], fill= 1)

def calcAngle2Vector(vec1, vec2, radian= False):
    '''
    Draw markers and segments between body parts according to predicted body part locations.
    Args:
        vec1: ndarray
        vec2: ndarray
        radian: boolean
            if True, angle calculated by radian, else degree
    Returns:
        angle: int
    ...
    angle = np.arccos(dot_product) if radian else np.degrees(np.arccos(dot_product))
    return angle

def genVector2D(point1, point2):
    '''
    Generate a vector corresponding to point1 and point2
    Args:
        point1: Tuple, List, or np array
            (point1_x, point1_y)
        point2: Tuple, List, or np array
            (point1_x, point1_y)
    Returns:
        vector: ndarray
            [vector_x, vector_y]
    '''
    return np.asarray(point2) - np.asarray(point1)